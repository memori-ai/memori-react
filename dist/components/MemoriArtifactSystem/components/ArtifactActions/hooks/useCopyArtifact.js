...
const useCopyArtifact = (artifact, onCopy, onDownload, onPrint) => {
    ...
    const handleCopy = (0, react_1.useCallback)(async (format) => {
        setCopyState(prev => ({
            ...prev,
            loading: true,
            error: null,
        ...
        try {
            if (format.action === 'pdf') {
                ...
                try {
                    await PDFExporter_1.pdfExporter.exportAsPDF(artifact.content, artifact.title || 'Artifact', pdfOptions);
          ...
          `;
                ...
                a.href = url;
                a.download = `${artifact.title || 'Artifact'}.${mimeType}`;
                a.click();
...
exports.useCopyArtifact = useCopyArtifact;
//# sourceMappingURL=useCopyArtifact.js.map