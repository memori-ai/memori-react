...
const UploadButton = ({ authToken = '', client, sessionID = '', baseUrl = '', isMediaAccepted = false, setDocumentPreviewFiles, documentPreviewFiles, memoriID = '', maxTotalMessagePayload, maxDocumentsPerMessage = 10, maxDocumentContentLength = 300000, onUploadLoadingChange, }) => {
    ...
    const handleUnifiedFileSelection = useCallback((files) => {
        var _a, _b, _c, _d;
...
${file.textAssetUrl || ''}
    ...
    const handleDocumentLoadingChange = useCallback((loading, fileCount) => {
        setIsDocumentLoading(loading);
        setDocUploadingCount(loading ? fileCount !== null && fileCount !== void 0 ? fileCount : 1 : 0);
    }, []);
    const handleImageLoadingChange = useCallback((loading, fileCount) => {
    ...
    return (_jsxs("div", { className: cx('memori--unified-upload-wrapper', {
            'memori--dragging': isDragging,
        }), ref: wrapperRef, children: [_jsx("input", { ref: unifiedInputRef, type: "file", accept: [
                    '.jpg',
                    '.jpeg',
                    '.png',
                    ...documentConversionExtensions,
